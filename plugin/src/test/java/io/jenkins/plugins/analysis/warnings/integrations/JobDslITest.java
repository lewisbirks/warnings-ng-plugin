...
class JobDslITest extends IntegrationTestWithJenkinsPerTest {
    /**
     * Creates a view from a YAML file and verifies the issues total column is visible.
     */
    @Test
    void shouldCreateColumnFromYamlConfiguration() {
        configureJenkins("column-total-dsl.yaml");
        ...
        assertThat(view.getColumns()).first()
                .isInstanceOfSatisfying(IssuesTotalColumn.class,
                        c -> assertThat(c)
                                ...
                                .hasName("Total Issues")
                                .hasNoTools()
                                .hasType(StatisticProperties.NEW));
    }
